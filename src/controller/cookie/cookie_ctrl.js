...
const save2 = (req,res)=>{
    ...
    cart_list =ser.save2(cart_list,req.params.goods); // 해당하는 정보와 아이디값(req.params.goods)을 넘겨줌
...
const viewList =(req,res)=>{
    let cart_list = req.signedCookies.cart_list;
    if(!cart_list){ //=== undefined){ //undefined == false >> cart_list에 값이 없다면
        const msg =`<script>
        alert("저장된 목록이 없습니다.");
        location.href ="/cookie/cart";
        </script>`;
...
module.exports={viewList, save1, save2, cart, makeCookie, index, popup, quiz, quizPopup};